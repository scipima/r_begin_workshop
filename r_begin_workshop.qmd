...
ggplot(data = gapminder,
       mapping = aes(x = gdpPercap,
                     y = lifeExp)) + geom_point(alpha = 0.3) +
...
summarise(n_votes = n(), # count votes
...
data_in |> 
dplyr::summarise(n_votes = n(),
n_yes = sum(vote == "yes", na.rm = TRUE)) |> 
dplyr::filter(n_votes >= min_tot_votes) |> 
dplyr::mutate(pct_yes = n_yes / n_votes) |> 
dplyr::arrange(dplyr::desc(pct_yes))
}
un_votes |> 
group_by(country) |> 
summarise_votes() |>  # test, we should get the same answer as above ...
ungroup()
```


### Trends over time in `yes` votes
Similar to the previous case - the `gapminder` data - we may be interested in the evolutions over time. 
In order to get that piece of info, we need to introduce another topic, namely **merges** (or **joins**).
Indeed, that dates are recorded in the other dataset, `un_roll_calls`.
Notice `left_join`.
```{r}
un_votes <- un_votes |> 
dplyr::left_join(
y = un_roll_calls |> 
dplyr::select(rcid, date),
by = "rcid")
...
However, depending on the country, the plot may be similar or very off from the aggregate.
...
geom_line(aes(x = year, y = pct_yes, # we're passing aes directly to geoms
group = country, colour = country)) + 
# here we're providing new data, aggregated on the fly
geom_line(data = un_votes |> 
dplyr::mutate(year = lubridate::year(date)) |> 
dplyr::group_by(year) |> 
summarise_votes() |> 
ungroup(),
...
theme_minimal() +
theme(legend.position = "top")
ggsave(here::here("figures", "unvotes_yesbycountry.png"), dpi=300, height=6, width=8, device="png", bg="white")
ggsave(here::here("figures", "unvotes_yesbycountry.png"), dpi=300, height=6, width=8, device="png", bg="white")
```
...
Extract the year feature, similar to what we id above, so that we can group the data afterwards.  
```{r}
# extract year
...
```{r}
plenary_docs |> 
dplyr::filter(grepl(pattern = "Committee on ",
...
dplyr::mutate(document_creator_organization = trimws(document_creator_organization)) |> 
dplyr::group_by(document_creator_organization) |> 
...
document_creator_organization, doc_count, .fun = max) ) |> 
...
facet_wrap(~document_creator_organization, ncol = 4,
...
janitor::clean_names() # |>  dplyr::filter( fieldwork_start >= as.Date(date_range_min) )
# str(ee)
...
dplyr::mutate(fieldwork_start = as.Date(fieldwork_start),
...
& party_ee %in% c("af_d", "cdu", "cdu_csu", "cdu", "fdp", "fw", "grune", 
"link", "spd")
