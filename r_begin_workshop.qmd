...
* Visualise data: choose an appropriate visualisation for your data; understand the layered approach typical of `ggplot2`.
...
Depending on the overall level, we might not cover all the content - that's ok, you still get this document if you want to explore further. 
...
We can explore the data in a more structure way, by checking what classes do we have as columns.
```{r}
#| code-fold: show
...
```{r}
rm(un_roll_calls, un_votes, summarise_votes); gc()
```
