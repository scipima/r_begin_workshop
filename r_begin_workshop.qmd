...
We can explore the data in a more structure way, by checking what classes do we have as columns, and get some descriptive statistics.
```{r}
#| code-fold: show
...
First, our visualisation obviously brings the reader's attention to the blue, grouped trend line by continent.
...
Second, pay attention to the larger variation surrounding the trend line in continents such as Asia and Africa compared to the relatively tighted distribution in Europe.
Inter alia, this would have substantial consequences were we to fit a model to this data: the larger variance in the former continents would mean larger uncertainty in our estimates compared to the European case.
```{r}
rm(p)
```
